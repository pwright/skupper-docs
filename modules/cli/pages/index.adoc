...
[id="skupper-cli"] 
= Configuring {skupper-name} sites using the CLI
...
+
--
include::example$ref.txt[tags=init]
--
====
...
+
---
For simple use cases, for example if you have a one-to-one relationship between service and port, you can combine the two steps above using the `expose` option:
+
----
